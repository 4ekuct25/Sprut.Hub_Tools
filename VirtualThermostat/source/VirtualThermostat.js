...
info = {
    ...
    options: {
        ...
        acPowerSwitch: {
            ...
            desc: {
                ru: "Опционально. Для кондиционеров, которые не выключаются режимом «Выключено» в сервисе Термостат (например, VIOMI) — у них питание на отдельном сервисе с характеристикой «Включен». Укажите его здесь: сценарий будет включать/выключать кондиционер через этот выключатель. Ручное включение/выключение им синхронизируется с термостатом.",
        ...
        acSmoothFactor: {
            ...
            desc: {
                ru: "Множитель к «Плавной целевой температуре»: насколько резко кондиционер набирает холод/тепло. Целевая кондея = его собственная температура + этот коэффициент × отставание комнаты от цели. 1.0 — мягко (по умолчанию), 2.0 — бодрее, 3.0+ — близко к форсажу. Помогает, когда в жару комната уходит выше цели. Переохладить нельзя — охлаждение всё равно выключается по гистерезису. Работает только с «Плавной целевой температурой».",
                en: "Multiplier for 'Smooth AC target temperature': how aggressively the AC pulls toward the goal. AC target = its own temperature + this factor × the room-to-goal gap. 1.0 — gentle (default), 2.0 — stronger, 3.0+ — near full blast. Helps when the room drifts above the goal on hot days. Can't overcool — cooling still stops by hysteresis. Active only with 'Smooth AC target temperature'."
            },
            type: "Double",
            value: 1.0,
            minValue: 1.0,
        ...
        acAnticipate: {
            ...
            desc: {
                ru: "На сколько градусов ВЫШЕ цели глушить компрессор (при нагреве — ниже). Он останавливается чуть раньше, и остаточный холод змеевика доводит комнату ровно до цели, не протягивая ниже. Вентилятор продолжает работать. Лечит переохлаждение на прохладных днях. Разумно 0.1–0.3; больше, чем комната поднимается над целью, ставить нельзя — компрессор не запустится. 0 — выключено (по умолчанию). Нужна «Плавная целевая температура».",
                en: "How many degrees ABOVE the goal to stop the compressor (below for heating). It stops a bit early and the residual coil cold brings the room exactly to the goal without dragging it lower. The fan keeps running. Fixes overcooling on cool days. Reasonable 0.1–0.3; don't set it larger than how far the room rises above the goal, or the compressor won't start. 0 disables it (default). Needs 'Smooth AC target temperature'."
            },
            type: "Double",
            value: 0.0,
            minValue: 0.0,
        ...
        acFanOnlyAtTarget: {
            ...
            desc: {
                ru: "Если включено: достигнув цели, кондиционер не выключается — компрессор останавливается, а вентилятор продолжает перемешивать воздух. Работает только в рабочей зоне и в заданном интервале времени, с гистерезисом: запускается по достижении коридора (сам по времени при холодной комнате не включается), а если кондей уже работал — удерживается ещё на гистерезис ниже, чтобы проскок охлаждения его не выключил. Глубже — полный выкл. Выключено — кондиционер гасится полностью (как раньше).",
                en: "If on: after reaching the goal the AC isn't turned off — the compressor stops but the fan keeps circulating air. Runs only in the working zone and the set time interval, with hysteresis: it starts on reaching the corridor (won't switch on by time alone when the room is cold), and if the AC was already running it's held another hysteresis lower so a cooling overshoot won't stop it. Deeper — full off. Off — the AC turns off completely (as before)."
            },
            type: "Boolean",
            value: false
        },
        acModulateAtTarget: {
            ...
            desc: {
                ru: "Уточняет «вентилятор без компрессора»: в той же зоне у цели кондею ставится не обдувная целевая, а плавная каскадная. Комната ≤ цели — целевая выше собственной, компрессор сам стоит (как обдув); чуть выше цели — мягко подхватывает охлаждение, не дожидаясь верха коридора. Так «пила» внутри гистерезиса меньше: вместо вкл/выкл компрессор плавно модулируется. Нужны «Плавная целевая температура» и «вентилятор без компрессора» (он задаёт окно и зону).",
                en: "Refines 'fan without compressor': in the same at-goal zone the AC gets the smooth cascade target instead of the fan-only one. Room ≤ goal — the target is above its own and the compressor idles (like fan-only); just above the goal it gently resumes cooling without waiting for the top of the corridor. So the sawtooth inside the hysteresis is smaller: instead of on/off the compressor modulates smoothly. Needs 'Smooth AC target temperature' and 'fan without compressor' (which sets the window and zone)."
            },
            type: "Boolean",
            value: false
        },
        ...
        failureBehavior: {
            ...
            desc: {
                ru: "Что делать, если датчик температуры молчит дольше заданного времени.\n• 'Отключить' — термостат в Выключен, оба реле OFF.\n• 'Нагрев' — режим не меняется, включается только реле нагрева.\n• 'Охлаждение' — режим не меняется, только реле охлаждения.\n• 'Ничего не делать' — состояние не трогается.\nКогда датчик оживёт — управление возвращается в обычный режим.",
                en: "What to do if the temperature sensor is silent longer than the set time.\n• 'Turn off' — thermostat to Off, both relays off.\n• 'Heat' — mode unchanged, only the heating relay on.\n• 'Cool' — mode unchanged, only the cooling relay on.\n• 'Do nothing' — state untouched.\nWhen the sensor recovers, control returns to normal."
            },
            type: "Integer",
            value: 0,
            formType: "list",
